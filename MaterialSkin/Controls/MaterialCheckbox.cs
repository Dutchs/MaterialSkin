...
namespace MaterialSkin.Controls
{
    public class MaterialCheckBox : CheckBox, IMaterialControl
    {
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            Color color = Color.FromArgb(colorAlpha, Enabled ? SkinManager.ColorScheme.AccentColor : SkinManager.GetCheckBoxOffDisabledColor());
            ...
            if (Ripple && rippleAnimationManager.IsAnimating())
            {
                for (int i = 0; i < rippleAnimationManager.GetAnimationCount(); i++)
                {
                    ...
                    var rippleColor = Color.FromArgb((int)((animationValue * 40)), ((bool)rippleAnimationManager.GetData(i)[0]) ? Color.Black : color);
            ...
            {
                Color blendedColor = DrawHelper.BlendColor(Parent.BackColor, Enabled ? SkinManager.GetCheckboxOffColor() : SkinManager.GetCheckBoxOffDisabledColor(), backgroundAlpha);
                using (var blendedBrush = new SolidBrush(blendedColor))
                using (var blendedPen = new Pen(blendedColor))
                {
                    g.FillPath(blendedBrush, checkmarkPath);
                    g.DrawPath(blendedPen, checkmarkPath);
                }

                using (var backBrush = new SolidBrush(Parent.BackColor))
                using (var backPen = new Pen(Parent.BackColor))
                {
                    g.FillRectangle(backBrush, boxOffset + 2, boxOffset + 2, CHECKBOX_INNER_BOX_SIZE - 1, CHECKBOX_INNER_BOX_SIZE - 1);
                    g.DrawRectangle(backPen, boxOffset + 2, boxOffset + 2, CHECKBOX_INNER_BOX_SIZE - 1, CHECKBOX_INNER_BOX_SIZE - 1); 
                }
                ...
                using (var checkmarkBitmap = DrawCheckMarkBitmap())
                {
                    g.DrawImageUnscaledAndClipped(checkmarkBitmap, checkMarkLineFill);
                }
            }
